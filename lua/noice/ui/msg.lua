...
function M.on_show(event, kind, content, replace_last)
  ...
  if kind == M.kinds.search_count then
    ...
    -- "[1/12]/foo" but the pattern is already visible in the cmdline
    -- view as the user types it; rendering it again as virtualtext
    -- next to the cursor is redundant clutter.
    if type(content) == "table" then
      local full = ""
      for _, c in ipairs(content) do
        if type(c) == "table" and c[2] then
          full = full .. c[2]
        end
      end
